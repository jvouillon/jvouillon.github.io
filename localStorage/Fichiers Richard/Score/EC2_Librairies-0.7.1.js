                ...
                if (b) {
                    ...
                    switch (t.charAt(n)) {
                        ...
                        case "@":
                            ...
                            break;
                        case "!":
                            r = new Date((x("!") - this._ticksTo1970) / 10000), m = r.getFullYear(), g = r.getMonth() + 1, v = r.getDate();
                            break;
                        case "'":
                            _("'") ? k() : b = !0;
                            break;
...
function play(str) {
    EdgeReady = new Variable(0, function() {
        dataInsert(templateData);
        playCCG()
    });
    if (mainStage) {
        playCCG()
...
$(function() {
    $("head").append("<style> .hidden {display:none;} button:focus {outline:0 !important;} </style>");
    $("head").append("<style> #debug{ display: inline-block; width: auto; background: #242424; font-family: Verdana,Arial; padding:5px; padding-right: 0px; } .debug_header{ color: yellow; font-size: 10px; margin-bottom: 3px; } .debug_header2{ color: yellow; font-size: 10px; margin-top: 5px; } #debug button { focus:none; background: -webkit-linear-gradient(#ffffff 0%, #ffffff 0%, #a4a4a4 48%, #ffffff 100%); border: none; color: white; text-shadow: 1px 1px 2px black; border-radius: 5px; margin-left:0px; } #debug_updateBtn { background: -webkit-linear-gradient(#25dd3d 0%, #00a54b 100%) !important; } #debug_playBtn { background: -webkit-linear-gradient(#25dd3d 0%, #00a54b 100%) !important; } #debug_nextBtn { background: -webkit-linear-gradient(#2573de 0%, #0059a6 100%) !important; } #debug_stopBtn { background: -webkit-linear-gradient(#de2525 0%, #a60000 100%) !important; } #debug_invokeBtn { background: -webkit-linear-gradient(#25dd3d 0%, #00a54b 100%) !important; } #debug_removeBtn { background: -webkit-linear-gradient(#dbde25 0%, #9ba600 100%) !important; } #debug_toggledataBtn { background: -webkit-linear-gradient(#eff313 0%, #bcad03 100%) !important; } #debug_cleardataBtn {float:right; margin-right:10px; margin-top:3px; background: -webkit-linear-gradient(#de2525 0%, #a60000 100%) !important; } #debug_defaultdataBtn { float:right; margin-right:5px; margin-top:3px; background: -webkit-linear-gradient(#2573de 0%, #0059a6 100%) !important; } #debug_invoke_string {  border: 1px solid #949494; background: black; color: white; padding: 2px; outline: none; } #debug textarea { margin-top:5px; border: none; background: black; border: 1px solid #949494; padding: 5px; width: calc(100% - 20px); height:200px; color: white; outline: none; } #debug_addcomponentDataBtn { background: -webkit-linear-gradient(#25dd3d 0%, #00a54b 100%) !important; }</style");
    $("head").append("<style> .autoscale{overflow: hidden !important; white-space: nowrap !important;} </style>");
    $("head").append("<style> .uppercase {text-transform:uppercase !important;} .lowercase {text-transform: lowercase !important;} .capitalize {text-transform:capitalize !important;} </style>");
    if (debug_buttons) {
        addDebugButtons()
    }
    $.ajaxPrefilter(function(options) {
        if (options.useProxy) {
            options.url = data_proxyUrl
        }
    })
});

function saveAutoScaleSizes() {
    $(".autoscale").each(function(index) {
        var currentSize = parseInt($(this).css("font-size"), 10);
        var elemID = $(this)[0].id;
        var obj = {};
        obj.element = elemID;
        obj.size = currentSize;
        autoscale_sizes.push(obj)
    })
}

function addDebugButtons() {
    $("body").append("<div id='debug'> <div class='debug_header'>E2C DEBUG</div> <button id='debug_playBtn' onclick='play();'>PLAY</button> <button id='debug_nextBtn' onclick='next();'>NEXT</button> <button id='debug_stopBtn' onclick='stop();'>STOP</button> <button id='debug_removeBtn' onclick='debug_remove();'>REMOVE</button> <button id='debug_invokeBtn' onclick='debug_invoke();'>INVOKE</button> <input id='debug_invoke_string' type='text' size=60 placeholder='type/paste invoke command here..'> <button id='debug_toggledataBtn' onclick='debug_toggledata();'>SHOW DATA</button>  <button id='debug_updateBtn' onclick='debug_update();'>UPDATE</button> <br> <div id='debug_data' class='hidden'> <div class='debug_header2'>TEMPLATE DATA</div> <textarea id='debug_update_string'></textarea><br><button id='debug_cleardataBtn' onclick='debug_cleardata();'>CLEAR DATA</button><button id='debug_defaultdataBtn' onclick='debug_defaultdata();'>DEFAULT DATA</button> <button id='debug_addcomponentDataBtn' onclick='debug_addComponent();'>ADD COMP.DATA</button>  </div> </div>");
    $("#debug_update_string").val('<templateData>\n\t<componentData id="f0">\n\t\t<data id="text" value="Text for F0" />\n\t</componentData>\n\t<componentData id="f1">\n\t\t<data id="text" value="Text for F1" />\n\t</componentData>\n</templateData>');
    $("#debug_update_string").keydown(function(e) {
        if (e.keyCode === 9) {
    ...
    if (debug_floating) {
        ...
        $("#debug").css("left", 10);
        $("#debug").css("top", 10);
        $("#debug").css("background", "rgba(20,20,20,0.8)");
        $("#debug_update_string").css("background", "rgba(20,20,20,0.8)");
        $("#debug").css("border", "1px solid black");
        $("#debug").css("border-radius", "5px");
        $(".debug_header").html("E2C DEBUG - DRAGGABLE!")
    }
}

function debug_remove() {
    remove()
}

function debug_invoke() {
    invoke($("#debug_invoke_string").val())
}

function debug_update() {
    update($("#debug_update_string").val())
}

function debug_toggledata() {
    $("#debug_data").toggleClass("hidden");
    if ($("#debug_data").hasClass("hidden")) {
        $("#debug_toggledataBtn").html("SHOW DATA")
    } else {
        $("#debug_toggledataBtn").html("HIDE DATA")
    }
}

function debug_cleardata() {
    $("#debug_update_string").val("")
}

function debug_defaultdata() {
    $("#debug_update_string").val('<templateData>\n\t<componentData id="f0">\n\t\t<data id="text" value="Text for F0" />\n\t</componentData>\n\t<componentData id="f1">\n\t\t<data id="text" value="Text for F1" />\n\t</componentData>\n</templateData>')
}

function debug_addComponent() {
    var newpart = '<componentData id="">\n\t\t<data id="text" value="" />\n\t</componentData>';
    var cursorPos = $("#debug_update_string").prop("selectionStart"),
        v = $("#debug_update_string").val(),
        textBefore = v.substring(0, cursorPos),
        textAfter = v.substring(cursorPos, v.length);
    $("#debug_update_string").val(textBefore + newpart + textAfter)
}
...
function parseCaspar(str) {
    ...
    if (window.DOMParser) {
        ...
        xmlDoc = parser.parseFromString(clean_str, "text/xml")
    }
    var dataFromClient = XML2JSON(xmlDoc.documentElement.childNodes);
    $.extend(templateData, dataFromClient);
    if (data_clientDataAsExternalParameters) {
        var ajaxData = templateData
    } else {
        ajaxData = {}
    }
    if (data_useExternal && data_externalType == "XML") {
        ajaxData.csurl = data_externalUrl;
        $.ajax({
            url: data_externalUrl,
            cache: false,
            data: ajaxData,
            useProxy: data_useProxy,
            success: function(data) {
                if (data_typeCasparXML) {
                    var clean_str = data.replace(/\t/g, "");
                    clean_str = clean_str.replace(/\n/g, "");
                    var xmlDoc;
                    if (window.DOMParser) {
                        parser = new DOMParser();
                        xmlDoc = parser.parseFromString(clean_str, "application/xml")
                    }
                    var dataFromExternal = XML2JSON(xmlDoc.documentElement.childNodes);
                    $.extend(templateData, dataFromExternal)
                } else {
                    var clean_str = data.replace(/\t/g, "");
                    clean_str = clean_str.replace(/\n/g, "");
                    var jsonData = x2js.xml_str2json(clean_str);
                    $.extend(templateData, jsonData)
                }
                dataInsert(templateData);
                updateExternal()
            },
    ...
    if (data_useExternal && data_externalType == "JSONP") {
        $.ajax({
            url: data_externalUrl,
            data: ajaxData,
            cache: false,
            jsonpCallback: data_jsonpCallback,
            success: function(data) {
    ...
    if (data_useExternal && data_externalType == "JSON") {
        $.ajax({
            url: data_externalUrl,
            data: ajaxData,
            cache: false,
...
function findDataWithAttr(from, searchAttr, searchValue, returnWhich, returnAttr) {
    try {
        from = eval("templateData." + from);
        var res = getObjects(from, searchAttr, searchValue);
        if (returnWhich == "*") {
            var rarr = [];
            res.forEach(function(entry) {
                var data = eval("entry." + returnAttr);
                rarr.push(data)
            });
            return rarr
        } else {
            var rdata = eval("res[" + returnWhich + "]." + returnAttr);
            return rdata
        }
    } catch (err) {}
}

function dataInsert(templateData) {
    for (var idCaspar in templateData) {
        var idTemplate = document.getElementById(idCaspar);
        if (idTemplate == undefined) {
            var idTemplate = document.getElementById("Stage_" + idCaspar)
        }
        if (idTemplate != undefined) {
            var text = templateData[idCaspar];
            var splitted = templateData[idCaspar].split(";");
            for (var x = 0; x < splitted.length; x++) {
                var partsplit = splitted[x].split("=");
                try {
                    if (partsplit[0].toUpperCase() == "IMG") {
                        $(idTemplate).css("background-image", "url('" + partsplit[1] + "')");
                        text = ""
                    }
                    if (partsplit[0].toUpperCase().trim() == "MASK") {
                        $(idTemplate).css("mask-image", "url('" + partsplit[1] + "')");
                        $(idTemplate).css("mask-size", "100% 100%");
                        text = ""
                    }
                } catch (e) {}
            }
            $(idTemplate).html(text)
        }
    }
    for (var idMapping in data_elementMapping) {
        var eData = null;
        if (typeof data_elementMapping[idMapping] === "object") {
            var haku = data_elementMapping[idMapping];
            if (haku.resultAttr == null) {
                haku.resultAttr = "__text"
            }
            if (haku.resultIndex == null) {
                haku.resultIndex = 0
            }
            eData = findDataWithAttr(haku.source, haku.attr, haku.search, haku.resultIndex, haku.resultAttr)
        }
        try {
            if (eData == null) {
                eData = eval("templateData." + data_elementMapping[idMapping])
            }
            var idTemplate = document.getElementById(idMapping);
            if (idTemplate == undefined) {
                var idTemplate = document.getElementById("Stage_" + idMapping)
            }
            if (idTemplate != undefined) {
                var text = eData;
                var splitted = eData.split(";");
                for (var x = 0; x < splitted.length; x++) {
                    var partsplit = splitted[x].split("=");
                    try {
                        if (partsplit[0].toUpperCase() == "IMG") {
...
function updateData(id, data) {
    var idTemplate = document.getElementById("Stage_" + id);
    if (idTemplate == undefined) {
        var idTemplate = document.getElementById(id)
    }
    if (idTemplate != undefined) {
        $(idTemplate).html(data)
    }
    updateAutoScale()
}
...
function updateImage(id, image, maskImage) {
    maskImage = typeof maskImage !== "undefined" ? maskImage : null;
    var idTemplate = document.getElementById("Stage_" + id);
    if (idTemplate == undefined) {
        var idTemplate = document.getElementById(id)
    }
    if (idTemplate != undefined) {
        $(idTemplate).css("background-image", "url('" + image + "')");
        if (maskImage != null) {
            $(idTemplate).css("mask-image", "url('" + maskImage + "')");
            $(idTemplate).css("mask-size", "100% 100%")
        }
    }
    updateAutoScale()
...
function edgeElement(id) {
    var element = document.getElementById("Stage_" + id);
    return $(element)
}
...
function updateAutoScale() {
    $(".autoscale").each(function(index) {
        for (var i = autoscale_sizes.length - 1; i >= 0; i--) {
            var o = autoscale_sizes[i];
            if (o.element == $(this)[0].id) {
                $(this).css("font-size", o.size + "px")
            }
        }
        var currentSize = parseInt($(this).css("font-size"), 10);
        var scaleDown = false;
...
// Initialization, run when Edge animations loaded & ready
// #######################################################
...
AdobeEdge.bootstrapCallback(function(compId) {
    ...
    AdobeEdge.Symbol.bindElementAction(compId, 'stage', 'document', 'compositionReady', function(sym, e) {
        ...
        yepnope({
            load: [
                'css/custom.css',
                'js/libs/gasp/TweenMax.min.js',
                'js/libs/gasp/jquery.gsap.min.js',
                'js/libs/gasp/plugins/ScrollToPlugin.min.js',
                'js/libs/gasp/plugins/RaphaelPlugin.min.js',
                'js/libs/gasp/plugins/KineticPlugin.min.js',
                'js/libs/gasp/plugins/ColorPropsPlugin.min.js',
                'js/libs/gasp/plugins/CSSRulePlugin.min.js',
                'js/libs/gasp/plugins/EaselPlugin.min.js',
                'js/libs/gasp/plugins/TextPlugin.min.js',
                'js/libs/moment.js',
                'js/libs/socket.io.js',
                'js/libs/velocity.min.js',
                'js/libs/velocity.ui.min.js',
        ...
        mainStage = comp.getStage();

        EdgeReady.SetValue(1);
